...
7. Publicar `update-manifest.json` em UTF-8 **sem** BOM, com schema, repositório, versão, nome, bytes, SHA-256 e notas em `pt-BR`, `en-US` e `es-ES`. O parser do launcher ignora um BOM UTF-8 se um asset antigo ainda o trouxer.
