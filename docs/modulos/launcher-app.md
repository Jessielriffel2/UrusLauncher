...
| `src/LegendLauncher.App/App.xaml.cs:16` | `OnStartup(...)` / `OnExit(...)` | Mantém um mutex local à sessão; a primeira instância liga o log de falhas, lê/aplica idioma e cria a janela, enquanto outra tentativa localiza o mesmo executável na sessão, restaura apenas se minimizado, ativa a janela existente e encerra. Na saída, libera o mutex. |
| `src/LegendLauncher.App/App.xaml.cs:70` | `InitializeDiagnosticLogging()` | Liga o log de arquivo em `Documentos/uruslauncher/logs` e captura exceções não tratadas do processo, dispatcher e tarefas. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Logging.cs:7` | `LogFailure(...)` | Encaminha falhas de catálogo, login rejeitado, runtime, persistência, idioma, doação e atualização ao `IDiagnosticLog`. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Launch.cs:8` | `StartGameAsync()` / `LaunchAsync(...)` | Extrai o fluxo de abertura; registra senha obrigatória, autenticação rejeitada, cancelamento e falha de start com código/mensagem/diagnóstico, sem senha. |
| `src/LegendLauncher.App/LegendLauncher.App.csproj:13` | Marca e artefato | Empacota o PNG Urus, define `AssemblyName=UrusLauncher.App`, mantém `RootNamespace=LegendLauncher.App`, aplica metadados “Urus Launcher”, usa `urus-launcher.ico` e publica o apphost sem CET shadow stack (`CETCompat=false`) para ferramentas de memória x64 anexarem. |
...
| `src/LegendLauncher.App/MainWindow.xaml:104` | Superfície de três colunas | Grade responsiva do launcher; painel de contas começa por volta da linha 114, com busca instantânea de perfil, rolagem personalizada, criação, seleção, edição, exclusão e servidores recentes. Os cartões de atualização e cofre no rodapé da coluna são faixas compactas para a lista de perfis ocupar o restante da altura. |
| `src/LegendLauncher.App/MainWindow.xaml:311` | `UpdateStatusView` | Insere o cartão de atualização na base da coluna de perfis, sem deslocar catálogo ou sessão. |
| `src/LegendLauncher.App/MainWindow.xaml:387` | Catálogo/versões | Seletor de plataformas OAS/7wan, busca, refresh e status. A lista por volta da linha 407 usa o template compartilhado com selos por função e divisor localizado. As barras de busca usam `SearchInputStyle`: lupa à esquerda, placeholder alinhado à esquerda e cursor no início ao focar. |
...
| `src/LegendLauncher.App/MainWindow.xaml.cs:96` | Foco da busca | Esconde o placeholder ao clicar/focar e coloca o cursor no início do campo vazio (perfis e servidores). |
| `src/LegendLauncher.App/MainWindow.xaml.cs:128` | Menu do perfil | Oferece jogar, editar, excluir e selecionar um servidor recente persistido; `SyncRecentServers()` recarrega o histórico salvo antes de montar o menu. |
...
| `src/LegendLauncher.App/Services/SessionLaunchCoordinator.cs:41` | `LaunchAsync(...)` | Resolve senha digitada ou salva, autentica, inicia `IGameRuntime` e persiste perfil/UID/histórico/cofre. Saída: `SessionLaunchOutcome` com `GameSession`/perfil em sucesso ou diagnóstico limitado a fase/transporte/status em falha; se a etapa pós-runtime falhar, encerra o processo ainda não adotado. |
...
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.cs:550` | `StartGameAsync()` | Reutiliza somente perfil+plataforma+servidor idênticos; outro servidor ou variante autentica, usa o `EffectiveProfile`, registra nova sessão e limpa a senha transitória. Se o workspace não adotar o PID/HWND retornado, encerra o GameHost órfão. O núcleo de autenticação/adoção foi extraído em `LaunchAsync(SessionLaunchInput)`, reutilizado pelo relogar. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Relog.cs:23` | `RelogSessionAsync(...)` | Reabre o mesmo servidor a partir de uma aba ativa: encerra a sessão atual via `Workspace.RequestClose`, reconstrói o `SessionLaunchInput` com perfil/plataforma/servidor da própria sessão (senha vazia → cofre, `RememberPassword` verdadeiro) e reexecuta `LaunchAsync` sem alterar a seleção da UI. O handler `WorkspaceOnRelogRequested` é `async void` fire-and-forget. |
...
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Catalog.cs:124` | `RefreshRecentServers()` / `SyncRecentServers()` | Resolve até cinco servidores jogados a partir do histórico da plataforma atual, da plataforma do perfil e de `LastServerId`, cruzando com o catálogo carregado. |
| `src/LegendLauncher.App/Themes/Controls.xaml:76` | `SearchInputStyle` | Campo em pílula com ícone à esquerda, `PART_ContentHost` e `SearchWatermark` alinhados à esquerda; o placeholder some com foco ou texto. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Profiles.cs:9` | `LoadProfilesAsync(...)` | Ordena perfis por atualização, restaura o ID selecionado em settings e notifica `FilteredProfiles`. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Profiles.cs:39` | `ApplyLaunchedProfileAsync(...)` | Após uma abertura aceita, substitui o perfil em memória com o histórico recente mesmo se a gravação em disco falhar. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.cs:304` | `ProfileSearchText` / `FilteredProfiles` | Filtra a lista de contas por nome e login enquanto a pessoa digita, sem limpar a seleção quando o filtro esconde o perfil atual. |
| `src/LegendLauncher.App/Themes/Controls.xaml:599` | `UrusScrollBarStyle` | Barra de rolagem estreita, sem setas, com thumb ciano; aplicada à lista de perfis e à coluna de avatares do workspace. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Profiles.cs:207` | `PersistSelectedProfileAsync(...)` | Salva apenas o GUID do último perfil; falha não invalida a seleção em memória. |
...
| `src/LegendLauncher.App/Views/Updates/UpdateStatusView.xaml:19` | Cartão de status | Faixa compacta no canto inferior esquerdo: título e ação na mesma linha, detalhe em uma linha, progresso só quando há download. |
...
- A busca acima da lista filtra instantaneamente por nome e login; a barra de rolagem personalizada aparece quando há mais contas do que o painel comporta.
...
- O histórico é atualizado somente depois de autenticação e abertura aceitas; o menu ⋮ mostra esses IDs persistidos, com fallback para `LastServerId` quando o mapa da plataforma veio vazio.
...
3. A `LaunchSession` segue por Named Pipe ao GameHost; perfil, usuário e senha não atravessam essa fronteira.
   Em falha, `SessionLaunchOutcome` propaga somente `AuthenticationFailureDiagnostic`; URI, query, corpo, cookie, token e senha não entram no resultado, UI ou persistência.
...
- Novos GameHosts herdam a cultura pelo ambiente do processo filho. A escolha está documentada em [localizacao.md](localizacao.md) e [ADR-005](../decisoes/ADR-005-localizacao-dinamica.md).
...
`tests/LegendLauncher.Tests/App/` cobre view model principal, registry, perfis, histórico, catálogo, coordenação de abertura, cleanup de processo não adotado, settings, áudio, workspace, work area de janelas borderless e validação do attachment HWND/PID (estilo parked sem `WS_VISIBLE` e estacionamento em `DestroyWindowCore`). `LauncherCompositionTests.cs` fixa a prioridade do runtime embutido e os fallbacks configurado/Brov. `MainWindowViewModelTests.cs` fixa Reborn salvo → Classic Português S100, botão habilitado, alvo exato enviado à autenticação, sessões distintas por plataforma/servidor, filtro instantâneo de perfis por nome/login e servidores recentes a partir de `LastServerId` quando o mapa da plataforma está vazio. `ServerCatalogPresentationTests.cs` fixa o servidor do perfil no topo, o critério `StartTimeUtc` com desempate por `NumericId`, a coexistência dos papéis, a prioridade de seleção e o divisor recalculado pela busca. `MainWindowLayoutXamlTests.cs` fixa o template com os dois selos, a seção dos demais servidores, a apresentação honesta de prontidão, a busca de perfis alinhada à esquerda no `SearchInputStyle` e a barra de rolagem personalizada. Os testes do workspace fixam capacidades 1/2/4, grade adaptativa, seleção visível, destaque da conta ativa na lateral, agrupamento fundido no layout 2, ativação por alvo exato, detach/reattach idempotente com rollback, fechamento e mudo global.
