...
O módulo oferece abas por conta, seleção rápida, layouts de uma, duas ou quatro superfícies, janela desacoplada, encerramento individual, mudo global e relogar (re-login no mesmo servidor a partir da aba). O rodapé também oferece **Relogar** para a sessão selecionada, e as áreas livres do cabeçalho/workspace podem arrastar a janela sem capturar controles. A barra ocupa uma única linha de 44 px; controles e abas têm 34 px, a lista de abas rola horizontalmente, `+ CONTA` permanece visível fora dessa rolagem e 150 px são reservados para os caption buttons da janela principal. Na grade de quatro, a geometria ocupa a área disponível de forma adaptativa: uma sessão usa 1×1, duas usam 1×2 e três ou quatro usam 2×2. Não existe limite lógico de quatro sessões: o layout define quantas ficam visíveis simultaneamente, enquanto as demais continuam nas abas e na barra lateral. A identidade de uma sessão é a combinação exata de perfil, plataforma e servidor; iniciar novamente esse mesmo alvo seleciona a sessão existente, enquanto a mesma conta pode permanecer simultaneamente em outra versão ou servidor. Todos os rótulos próprios do workspace e das janelas desacopladas acompanham dinamicamente o módulo de [Localização](localizacao.md).
...
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:42` | `RelogSessionCommand` | `RelayCommand<GameSessionViewModel>` habilitado somente para sessões em execução; valida e dispara `RelogRequested` sem remover a sessão — o encerramento fica a cargo do orquestrador da App. |
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:291` | `RequestClose(...)` | Encerra/remove a sessão por solicitação externa (usado pelo relogar antes de disparar o novo login), preservando as demais abas. |
...
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:233` | `TryActivateSession(...)` | Localiza uma sessão em execução pela chave `ProfileId + PlatformId + ServerId`; seleciona apenas o mesmo alvo e permite que um perfil mantenha outros servidores ou versões em paralelo. |
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:252` | `Reattach(...)` | Traz uma sessão desacoplada de volta ao conjunto de superfícies incorporadas sem reiniciar o GameHost. |
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:303` | Fechamento e detach | Encerrar termina/remove a sessão; desacoplar apenas a marca fora do grid e emite `DetachRequested`. |
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:336` | Saída espontânea do processo | Redireciona o callback para a `Dispatcher` WPF, remove a sessão, desregistra seu áudio e seleciona outra sessão válida. |
| `src/LegendLauncher.App/ViewModels/GameWorkspaceViewModel.cs:368` | `RefreshVisibleSessions()` | Filtra sessões em execução/não desacopladas até a capacidade do layout e substitui a última posição quando necessário para manter a seleção visível. |
...
| `src/LegendLauncher.App/GameHosting/GameWindowAttachment.cs:34` | `AttachTo` / `ResizeTo` / `ParkIfParent` / `DetachIfParent` | Move a mesma janela GameHost entre superfícies visíveis e o HWND de estacionamento oculto; o detach para o desktop só restaura estilo sem `WS_VISIBLE` e re-oculta, para sessões vivas não reaparecerem como janela top-level. |
| `src/LegendLauncher.App/GameHosting/GameHostParkingSurface.cs:7` | `GameHostParkingSurface` | Cria e reutiliza um HWND popup 1×1 oculto no processo do launcher para manter GameHosts fora da grade como `WS_CHILD`. |
| `src/LegendLauncher.App/GameHosting/EmbeddedGameSurfaceHost.cs:12` | `EmbeddedGameSurfaceHost` | `HwndHost` que cria um HWND-proxy do launcher, anexa a janela externa, sincroniza o client rect após layout/`SizeChanged` (o mesmo recálculo que minimizar/restaurar) e estaciona a sessão viva ao desmontar o presenter. |
...
| `src/LegendLauncher.App/ViewModels/WorkspaceAvatarItem.cs:3` | `WorkspaceAvatarItem` | Item da barra lateral: uma conta ou o par fundido do layout 2. |
| `src/LegendLauncher.App/Views/Game/GameWorkspaceView.xaml:357` | Avatares laterais | A barra esquerda lista sessões com inicial; a conta ativa recebe borda ciano e `AutomationProperties.Name` com sufixo selecionado. No layout 2, as duas contas visíveis viram um agrupamento sobreposto; as demais continuam empilhadas. Clicar no grupo abre as duas em tela dividida. Com muitas contas, a lista rola. |
| `src/LegendLauncher.App/Views/Game/GameWorkspaceView.xaml:410` | Superfícies de jogo | Materializa `Workspace.VisibleSessions` na grade adaptativa abaixo da barra compacta. |
...
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.cs:550` | `StartGameAsync()` | Volta à aba somente para perfil + plataforma + servidor idênticos ou autentica/inicia outro GameHost, adota o `EffectiveProfile`, registra a nova sessão e abre o workspace; um processo não adotado é encerrado. O núcleo de autenticação/adoção foi extraído em `LaunchAsync(SessionLaunchInput)`, reutilizado pelo relogar. |
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.Relog.cs:23` | `RelogSessionAsync(...)` | Orquestra o relogar a partir de uma aba ativa: encerra a sessão atual via `RequestClose`, reconstrói um `SessionLaunchInput` com o perfil/plataforma/servidor da própria sessão (senha vazia → cofre, `RememberPassword` verdadeiro) e chama `LaunchAsync`, reabrindo o mesmo servidor sem alterar a seleção da UI. O handler `WorkspaceOnRelogRequested` é `async void` fire-and-forget e ignora cancelamento. |
...
5. Trocar layout ou seleção rematerializa somente as superfícies necessárias. Na grade, 1/2/3–4 sessões ocupam respectivamente 1×1, 1×2 e 2×2. Destruir um presenter **estaciona** a janela GameHost como filha de um HWND oculto do launcher; não a devolve visível ao desktop e não encerra o processo. O host recalcula `ResizeTo` depois do layout WPF para o HWND acompanhar o quadrado do jogo (minimizar/restaurar deixa de ser necessário para corrigir desencaixe).
...
7. Relogar (botão na aba e no cabeçalho do painel) dispara `RelogSessionCommand`, que emite `RelogRequested`. A App encerra a sessão atual, reconstrói o `SessionLaunchInput` a partir do perfil/plataforma/servidor da própria sessão e reexecuta o mesmo fluxo de login, reabrindo o mesmo servidor sem mexer na seleção da UI.

## Barra, chrome e responsividade

- No layout 2, as duas superfícies visíveis aparecem na barra esquerda como bolinhas fundidas; o layout 1 desfaz o grupo. Clicar no agrupamento restaura aquelas duas contas na tela dividida.
...
- Fechar uma aba encerra somente seu GameHost daquela conta. Saída espontânea remove a aba automaticamente pela `Dispatcher`. Fechar o launcher tenta fechar todas as janelas desacopladas sem reattach e termina todas as sessões rastreadas.
...
- `GameWorkspaceViewModelTests.cs` cobre capacidades 1/2/4, grade adaptativa 1×1/1×2/2×2, manutenção da seleção visível, detach/reattach sem remoção, identidade de sessão por perfil/plataforma/servidor, coexistência da mesma conta em alvos diferentes, mudo, fechamento, `RelogSessionCommand` (evento `RelogRequested`) e `RequestClose`.
- `MainWindowViewModelTests.cs` cobre o relogar de uma sessão ativa: encerra a sessão original, dispara um novo login no mesmo perfil/plataforma/servidor e valida a nova aba adotada sem duplicar a anterior.
