...
| `src/LegendLauncher.App/ViewModels/MainWindowViewModel.cs:550` | `StartGameAsync()` | Volta à aba somente para perfil + plataforma + servidor idênticos ou autentica/inicia outro GameHost, adota o `EffectiveProfile`, registra a nova sessão e abre o workspace; um processo não adotado é encerrado. |
