...
| `src/LegendLauncher.App/Localization/Resources/*.json:156` | Textos de apoio | 26 chaves `Donation_*` dentro dos 268 textos de cada catálogo: título, explicação, brincadeira dos diamantes, instruções PayPal/PIX, feedback de cópia, fechamento e automação em `pt-BR`, `en-US` e `es-ES`. |
...
- Os contratos de localização verificam as 268 chaves e referências XAML nos três idiomas.
