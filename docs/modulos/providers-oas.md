...
- `OasAuthenticationService()` — implementação de produção de `IGameAuthenticationService`. Cria `SocketsHttpHandler`, `HttpClient` e `CookieContainer` novos por tentativa; todos os Passports OAS (Creaction/Reborn e OAS Games) usam o wrapper curl estrito, e a entrada pós-Passport mantém o wrapper de launch. Referências: `src/LegendLauncher.Providers.Oas/OasAuthenticationService.cs:14`, `:45` e `:76`.
...
- `AuthenticateAsync(request, cancellationToken)` — valida UTF-16, plataforma, host e `/serverlist/s<dígitos>` antes da rede; executa Passport e resolução no mesmo jar isolado. Cancelamento, rede, HTTP, tamanho, payload e origem viram erros locais estáveis acompanhados somente por fase/transporte/status saneados. Referência: `src/LegendLauncher.Providers.Oas/OasAuthenticationService.cs:121`.
- `AuthenticatePassportAsync` — usa `https://passport.creaction-network.com/index.php` somente para `lortr` e `https://passport.oasgames.com/index.php` nas demais variantes. Todas as variantes OAS selecionam `SystemCurl` no serviço de produção, porque a borda Cloudflare passou a devolver `403` ao stack .NET em ambos os Passports. Ambos exigem origem efetiva exata, leem `val.loginKey`/`val.id` e criam o cookie seguro/HttpOnly `oas_user`. Referências: `src/LegendLauncher.Providers.Oas/OasAuthenticationService.cs:212`, `:491` e `:501`.
- `ResolveLaunchSessionAsync` — força `pay_later=1`, preserva os demais parâmetros e processa HTML/redirect controlado por no máximo dois saltos adicionais. `login?token` é follow-up; `game.jsp` vira `Loading.swf`; cada resposta atualiza apenas o diagnóstico limitado da fase Launch. Referência: `src/LegendLauncher.Providers.Oas/OasAuthenticationService.cs:285`.
...
## Transportes curl compatíveis e limitados

- `OasSystemCurlTransport` — executor compartilhado sem autoridade sobre destinos. Executa exclusivamente `Environment.SystemDirectory\curl.exe`, sem janela, com argumentos estáticos (`--disable`, IPv4, HTTP/1.1, HTTPS-only, zero redirects, tempo/tamanho) e recebe a configuração sensível apenas por `stdin`; `stderr` é drenado e descartado. Referência: `src/LegendLauncher.Providers.Oas/OasSystemCurlTransport.cs:13`.
- `OasCurlPassportTransport` — wrapper JSON que aceita somente HTTPS/443, sem userinfo/fragmento, nos host/path exatos `passport.creaction-network.com/index.php` e `passport.oasgames.com/index.php`, e com exatamente `m=login`, `email` e `pwd` na ordem construída pelo serviço. Não envia cookies e nunca coloca a URI com usuário/senha nos argumentos. Referência: `src/LegendLauncher.Providers.Oas/OasCurlPassportTransport.cs:17`.
- `OasCurlLaunchTransport` — wrapper HTML separado que continua aceitando somente origens de jogo permitidas e pode enviar apenas cookies transitórios do jar. Referência: `src/LegendLauncher.Providers.Oas/OasCurlLaunchTransport.cs:9`.
...
- `OasAuthenticationDiagnosticContext` — estado por tentativa que produz somente `AuthenticationFailureDiagnostic` com fase, transporte e status; não aceita texto ou endereço remoto. Referência: `src/LegendLauncher.Providers.Oas/OasAuthenticationDiagnosticContext.cs:8`.
...
3. A senha viaja somente na requisição Passport, sempre pela configuração em `stdin` do curl confiável do Windows (Creaction/Reborn e OAS Games). Nunca entra em argumentos, logs, UI, arquivos ou exceções; o curl de launch recebe somente URI/cookies transitórios pós-Passport.
...
- `OasAuthenticationServiceTests.cs` cobre seleção do transporte curl para Creaction/Reborn e OAS Games, diagnóstico fase/transporte/status, endpoints, escaping, `loginKey`/`id`, `oas_user`, isolamento concorrente, redirects/follow-ups, allowlist, limites, timeout, cancelamento e representações sem segredo.
...
- `OasCurlPassportTransportTests.cs` cobre os dois endpoints exatos (Creaction e OAS Games), rejeição de esquema/host/path/porta/userinfo/fragmento, `Accept: application/json`, cancelamento e ausência de URI/conta/senha nos argumentos.
...
Suíte padrão permanece determinística e não acessa credenciais reais. Em 26/07/2026, prova anônima sem query/cookie/corpo observou `403` no Passport Creaction pelo stack .NET e `200` pelo curl do Windows; OAS Games respondeu `200` em ambos, fundamentando a seleção limitada sem efetuar login. Em 20/08/2026, a mesma prova anônima passou a observar `403` também no Passport OAS Games pelo stack .NET (continuando `200` no curl), o que motivou estender o wrapper curl a todas as variantes OAS. Login real continua exclusivamente manual pelo usuário. O fluxo integrado anterior confirmou duas sessões simultâneas e embutidas carregando até 99%; nenhum teste automatizado registra senha, cookie ou URI autenticada.
